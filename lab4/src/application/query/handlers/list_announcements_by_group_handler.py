...
from ..list_announcements_by_group_query import ListAnnouncementsByGroupQuery


class ListAnnouncementsByGroupHandler:
    """Обработчик запроса списка объявлений в группе"""
    
    def __init__(self, repository: AnnouncementRepository):
        self._repository = repository
    
    def handle(self, query: ListAnnouncementsByGroupQuery) -> List[AnnouncementDTO]:
        """
        Получить список объявлений в группе
        
        Шаги:
        1. Найти объявления в репозитории
        2. Преобразовать в DTO
        """
        announcements = self._repository.find_by_group(
